...
var Request = (function () {
    ...
    Request.prototype.execute = function () {
        ...
        return new Promise(function (resolve, reject) {
            xhr.open(method, Utils.urlFromString(url, read ? agent.transform.parseRequestBody(params) : null), true);
            xhr.onload = function () {
                if (xhr.status >= 400)
                    return reject(Errors.create(xhr.status, xhr.responseText, agent.transform.parseResponseBody(xhr.response), xhr.response));
                return resolve(Response.create(xhr, ((xhr.response != null) && xhr.response !== '') ?
                    agent.transform.parseResponseBody(xhr.response) : undefined));
            };
            if (ttl > 0)
                xhr.timeout = ttl;
            exports.HEADERS.set(xhr, agent.headers, read ? (_a = {}, _a[Headers.ACCEPTS] = agent.transform.accepts, _a) : (_b = {}, _b[Headers.CONTENT_TYPE] = agent.transform.contentType, _b), headers);
